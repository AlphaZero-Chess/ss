...
// Opening books from different bot styles
const ELEGANT_OPENINGS = {
  "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -": {
    white: ["e2e4", "d2d4", "c2c4", "g1f3"],
    black: ["c7c5", "e7e5", "e7e6", "c7c6"]
  },
...
const NON_ELEGANT_OPENINGS = {
  "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -": {
    white: ["e2e4", "d2d4", "f2f4", "b2b3"],
    black: ["c7c5", "e7e5", "d7d5", "f7f5"]
  },
  "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3": {
    black: ["c7c5", "e7e5", "d7d5", "g8f6"]
  }
};

const MINI_OPENINGS = {
  "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -": {
    white: ["d2d4", "c2c4", "g1f3", "e2e4"],
    black: ["d7d5", "g8f6", "e7e6", "c7c6"]
  },
  "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3": {
    black: ["e7e6", "c7c6", "d7d5", "e7e5"]
  }
};
...
const ChessGame = ({ enemy, playerColor, onGameEnd, onBack }) => {
  ...
  const [boardSize, setBoardSize] = useState(280); // Default smaller size
  ...
  const makeEngineMove = useCallback((currentGame) => {
    ...
    if (currentGame.moveNumber() <= 10) {
      const bookMove = getBookMove(currentGame.fen(), engineColor);
      if (bookMove) {
        setTimeout(() => applyEngineMove(bookMove), 300 + Math.random() * 400);
        return;
      }
    }
  ...
  }, [getEngineSettings, playerColor, getBookMove, applyEngineMove]);
